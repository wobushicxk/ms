...
class MultiBoxLoss(nn.Cell):
    ...
    def forward(self,
        ...
        loc_t = Tensor(num, num_priors, 4)
        #conf_t = torch.LongTensor(num, num_priors)
        conf_t = Tensor(num, num_priors)
        ...
        loc_t = stop_gradient(loc_t)
        conf_t = stop_gradient(conf_t)
        ...
        #size_average已经弃用
        loss_sl = nn.SmoothL1Loss()
        loss_l = loss_sl(loc_p, loc_t)
        ...
        num_neg = ops.clip_by_value(self.negpos_ratio *num_pos, clip_value_max=pos.size(1) - 1)
        ...
        loss_ce = nn.SoftmaxCrossEntropyWithLogits()
        loss_c = loss_ce(conf_p, targets_weighted)
