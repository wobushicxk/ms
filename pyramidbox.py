...
class CPM(nn.Cell):
    ...
    def forward(self, x):
        ...
        x = nn.ReLU(self.branch2a(x))
        x = nn.ReLU(self.branch2b(x))
        x = self.branch2c(x)

        rescomb = nn.ReLU(x + out_residual)
        ssh1 = self.ssh_1(rescomb)
        ssh_dimred = nn.ReLU(self.ssh_dimred(rescomb))
        ssh_2 = self.ssh_2(ssh_dimred)
        ssh_3a = nn.ReLU(self.ssh_3a(ssh_dimred))
        ssh_3b = self.ssh_3b(ssh_3a)

        op = ops.Concat(1)
        ssh_out = op([ssh1, ssh_2, ssh_3b])
        ssh_out = nn.ReLU(ssh_out)
        return ssh_out
...
class PyramidBox(nn.Cell):
    ...
    def _upsample_prod(self, x, y):
        ...
        return nn.ResizeBilinear(x, size=(H, W)) * y
    ...
    def forward(self, x):
        ...
        x = nn.ReLU(self.lfpn_topdown[0](convfc_7))
        lfpn2_on_conv5 = nn.ReLU(self._upsample_prod(
            x, self.lfpn_later[0](conv5_3)))

        x = nn.ReLU(self.lfpn_topdown[1](lfpn2_on_conv5))
        lfpn1_on_conv4 = nn.ReLU(self._upsample_prod(
            x, self.lfpn_later[1](conv4_3)))

        x = nn.ReLU(self.lfpn_topdown[2](lfpn1_on_conv4))
        lfpn0_on_conv3 = nn.ReLU(self._upsample_prod(
        ...
        op = ops.Spilt(1,2)
        face_loc, head_loc = op(mbox_loc)

        #face_loc = face_loc.permute(0, 2, 3, 1).contiguous().view(N, -1, 4)
        permute = ops.Transpose()
        input_perm = (0,2,3,1)
        face_loc = permute(face_loc, input_perm).ravel().view(N, -1, 4)

        if not self.is_infer:
            #contiguous要用ravel
            #head_loc = head_loc.permute(0, 2, 3, 1).contiguous().view(N, -1, 4)
            hace_loc = permute(head_loc, input_perm).ravel().view(N, -1, 4)
        ...
        for i, feat in enumerate(inputs):
            ...
            if not self.is_infer:
                ...
                head_conf = permute(mbox_conf[:, 4:, :, :], input_perm).ravel().view(N, -1, 2)
        ...
        op = ops.Concat(1)
        face_mbox_loc = op(face_locs)
        face_mbox_conf = op(face_confs)

        if not self.is_infer:
            head_mbox_loc = op(head_locs)
            head_mbox_conf = op(head_confs)
    ...
    def load_weights(self, base_file):
        ...
        if ext == '.ckpt':
            ...
            mdata = load_checkpoint(base_file)
...
def vgg(cfg, i, batch_norm=False):
    ...
    layers += [conv6,
               nn.ReLU(inplace=True), conv7, nn.ReLU]
    return layers
...
if __name__ == '__main__':
    ...
    inputs = Variable(rand(1, 3, 640, 640))
